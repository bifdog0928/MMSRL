...
def train(adata,knn=10,h=[3000,3000], n_epochs=200,lr=0.0001, key_added='MMSRL', random_seed=110,res=1,
          l=2,weight_decay=0.0001,a=10,b=1,c=10,embed=True,radius=0,enhancement=False,cluster="kmeans",loss_type="consistency",
                device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu'), mask_loss_weight=1):
    ...
    model =models.MMSRL(nfeatX=features_X.shape[1],
    ...
    for epoch in range(n_epochs):
        model.train()
        optimizer.zero_grad()
        z_xi, q_x, q_i, pi, disp, mean, loss_mask, E_G, E_H, Z_G, Z_H, Z_F, Z = model(features_X, features_I, adj)
        ...
        total_loss = a * zinb_loss + b * cl_loss + mask_loss_weight * loss_mask + c * reg_loss
        # print(total_loss)
        total_loss.backward()
        optimizer.step()
        if epoch % 5 == 0:
        ...
        if 'ground_truth' in adata.obs.columns:
            ...
            if ari_res > ari_max:
                ...
                adata.obsm['q_x'] = q_x.to('cpu').detach().numpy()
                adata.obsm['q_i'] = q_i.to('cpu').detach().numpy()

                adata.obsm['E_G'] = E_G.to('cpu').detach().numpy()
                adata.obsm['E_H'] = E_H.to('cpu').detach().numpy()
                adata.obsm['Z_G'] = Z_G.to('cpu').detach().numpy()
                adata.obsm['Z_H'] = Z_H.to('cpu').detach().numpy()
                adata.obsm['Z_F'] = Z_F.to('cpu').detach().numpy()
                adata.obsm['Z'] = Z.to('cpu').detach().numpy()
...
def train_mul(adata,adjlist,knn=10,h=[3000,3000], n_epochs=200,lr=0.0001, key_added='MMSRL', random_seed=110,res=1,
          l=2,weight_decay=0.0001,a=10,b=1,c=10,embed=True,radius=0,enhancement=False,cluster="kmeans",loss_type="consistency",
